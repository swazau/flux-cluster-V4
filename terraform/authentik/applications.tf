resource "authentik_outpost" "proxyoutpost" {
  ...
  protocol_providers = [
    resource.authentik_provider_proxy.jellyseer.id,
  ]
  config = jsonencode({
...
resource "authentik_provider_proxy" "jellyseer" {
  name               = "jellyseer-provider"
  external_host      = "https://jsr.${data.sops_file.authentik_secrets.data["cluster_domain"]}"
  mode               = "forward_single"
  authorization_flow = resource.authentik_flow.provider-authorization-implicit-consent.uuid
}

resource "authentik_application" "jellyseer" {
  name              = "Jellyseer"
  slug              = "jsr"
  group             = "Media"
  protocol_provider = resource.authentik_provider_proxy.transmission.id
  meta_icon         = "https://github.com/Fallenbagel/jellyseerr/raw/develop/public/logo_full.svg"
